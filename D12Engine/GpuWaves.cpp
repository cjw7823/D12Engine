...
{
	// GpuWaves를 위해 힙에 예약할 디스크립터 수.
	return 6;
}

void GpuWaves::BuildResources(ID3D12GraphicsCommandList* cmdList)
{
	//wave 시뮬레이션에 사용하는 모든 텍스처는	이후에 SRV와 UAV 둘 다로 바인딩된다. (핑퐁 방식으로 버퍼를 번갈아 쓰기 때문)
	...
	//셰이더에서 출력 대상이 되므로 UAV. 버퍼를 돌려쓰는 구조(핑퐁 방식).
	CD3DX12_RESOURCE_BARRIER barrier1 = CD3DX12_RESOURCE_BARRIER::Transition(mPrevSol.Get(),
		D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_STATE_COPY_DEST);
	cmdList->ResourceBarrier(1, &barrier1);

	UpdateSubresources(cmdList, mPrevSol.Get(), mPrevUploadBuffer.Get(), 0, 0, num2DSubresources, &subResourceData);

	barrier1 = CD3DX12_RESOURCE_BARRIER::Transition(mPrevSol.Get(),
		D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
	cmdList->ResourceBarrier(1, &barrier1);

	//초기 프레임에서 현재 상태를 읽어야 하기 때문에 READ 상태로 복사.
	barrier1 = CD3DX12_RESOURCE_BARRIER::Transition(mCurrSol.Get(),
		D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_STATE_COPY_DEST);
	cmdList->ResourceBarrier(1, &barrier1);
	...
	barrier1 = CD3DX12_RESOURCE_BARRIER::Transition(mNextSol.Get(),
...
void GpuWaves::Update(const GameTimer& gt, ID3D12GraphicsCommandList* cmdList, ID3D12RootSignature* rootSig, ID3D12PipelineState* pso)
{
	...
	if (t >= mTimeStep)
	{
		...
		//mNumRows, mNumCols는 16으로 나누어떨어져야 나머지가 생기지 않는다.
		UINT numGroupsX = mNumCols / 16;
		UINT numGroupsY = mNumRows / 16;
		cmdList->Dispatch(numGroupsX, numGroupsY, 1);

		/*
			다음 업데이트를 준비하기 위해 버퍼를 ping-pong 한다.
		...
		*/
		...
		//현재 해는 vertex shader가 읽을 수 있어야 하므로, 상태를 GENERIC_READ로 바꾼다.
		TransitionIfNeeded(cmdList, mCurrSol.Get(), mCurrSolState, D3D12_RESOURCE_STATE_GENERIC_READ);
...
void GpuWaves::TransitionIfNeeded(ID3D12GraphicsCommandList* cmdList, ID3D12Resource* resource, D3D12_RESOURCE_STATES& currentState, D3D12_RESOURCE_STATES newState)
{
