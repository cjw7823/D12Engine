...
void RenderApp::BuildWorldRayFromScreen(int sx, int sy, DirectX::XMVECTOR& rayOriginW, DirectX::XMVECTOR& rayDirW) const
{
...
DirectX::XMVECTOR RenderApp::BuildDragPlaneNormal(DirectX::XMVECTOR axisW,	DirectX::XMVECTOR cameraForwardW) const
{
...
DirectX::XMVECTOR RenderApp::MakePlaneFromPointNormal(DirectX::XMVECTOR pointW,	DirectX::XMVECTOR normalW) const
{
...
bool RenderApp::IntersectRayPlane(DirectX::XMVECTOR rayOriginW, DirectX::XMVECTOR rayDirW, DirectX::XMVECTOR plane,	DirectX::XMVECTOR& hitPointW) const
{
...
void RenderApp::EndGizmoDrag()
{
	mGizmo.Dragging = false;
	mGizmo.ActiveAxis = GizmoAxis::None;
}

DirectX::XMVECTOR RenderApp::GetGizmoAxisVector(GizmoAxis axis) const
{
	switch (axis)
	{
	case GizmoAxis::X:
...
void RenderApp::SetSelectedObjectPositionW(const DirectX::XMFLOAT3& posW)
{
...
void RenderApp::SelectRenderItemByMouseClick(int sx, int sy)
{
	...
	BuildWorldRayFromScreen(sx, sy, worldRayOrigin, worldRayDir);
...
InstanceData* RenderApp::GetPrimarySelectedInstance()
{
...
float RenderApp::CalcGizmoAxisLength(const DirectX::XMFLOAT3& pivotW) const
{
	...
	float axisLength = worldScreenHeight * (desiredPixelLength / static_cast<float>(mClientHeight));
...
GizmoAxis RenderApp::PickGizmoAxis(int sx, int sy)
{
