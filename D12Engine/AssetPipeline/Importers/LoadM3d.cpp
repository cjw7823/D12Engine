...
bool M3DLoader::LoadM3d(const std::string& filePath, std::vector<BoneVertex>& vertices, std::vector<uint16_t>& indices, std::vector<Subset>& subsets, std::vector<M3dMaterial>& mats)
{
...
void M3DLoader::ReadVertices(std::ifstream & fin, UINT numVertices, std::vector<BoneVertex>&vertices)
{
	std::string ignore;
